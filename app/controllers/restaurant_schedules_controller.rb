class RestaurantSchedulesController < ApplicationController
  ...
  before_action :set_restaurant_schedule_and_check_restaurant, only: [ :show ]
  def new
    @restaurant_schedule = RestaurantSchedule.new
  end
  
  def create
    restaurant_schedule_params = params.require(:restaurant_schedule).permit(:mon_open, :mon_close, :tue_close,
